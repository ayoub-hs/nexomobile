        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (item.serverId != null && item.status == QueuedOrderStatus.SYNCED) {
                    ...
                    if (result.isFailure) {
                        val errorMsg = result.exceptionOrNull()?.message ?: "Échec de la suppression sur le serveur"
                        _state.value = _state.value.copy(
                            isDeleting = false,
                ...
                if (deleteResult.isFailure) {
                    throw deleteResult.exceptionOrNull() ?: Exception("Échec de la suppression locale")
                }
                ...
                _state.value = _state.value.copy(
                    isDeleting = false,
                    deleteError = e.message ?: "Échec de la suppression de la commande"
                )
            }
        }
    }
