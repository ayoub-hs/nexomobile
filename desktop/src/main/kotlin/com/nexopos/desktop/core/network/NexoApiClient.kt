...
class NexoApiClient(private val settings: AppSettings) {
    ...
    private val debugLogging: Boolean = isDebugLoggingEnabled()
        ...
        return try {
            ...
            response.use { resp ->
                if (resp.isSuccessful) {
                    ...
                    logDebug("[NexoApiClient] Response: ${body.take(200)}...")
                    val data = adapter.fromJson(body) ?: throw Exception("Failed to parse response")
                    Result.success(data)
                } else {
                    logDebug("[NexoApiClient] HTTP Error: ${resp.code} ${resp.message}")
                    Result.failure(Exception("HTTP ${resp.code}: ${resp.message}"))
                }
            }
        } catch (e: Exception) {
            logDebug("[NexoApiClient] Exception: ${e.message}")
            Result.failure(e)
        }
    }
    ...
    suspend fun <T, R> post(endpoint: String, body: T, bodyAdapter: JsonAdapter<T>, responseAdapter: JsonAdapter<R>): Result<R> {
        return try {
            ...
            logDebug("[NexoApiClient] POST $url")
            logDebug("[NexoApiClient] Body: ${json.take(200)}...")
            ...
            response.use { resp ->
                ...
                logDebug("[NexoApiClient] Response ${resp.code}: ${responseBody.take(200)}")
            ...
            logDebug("[NexoApiClient] POST error: ${e.message}")
            Result.failure(e)
        }
    }

    suspend fun <T, R> put(endpoint: String, body: T, bodyAdapter: JsonAdapter<T>, responseAdapter: JsonAdapter<R>): Result<R> {
        return try {
            val url = "${settings.baseUrl}/$endpoint"
            val json = bodyAdapter.toJson(body)
            logDebug("[NexoApiClient] PUT $url")
            logDebug("[NexoApiClient] Body: ${json.take(200)}...")
            val requestBody = json.toRequestBody("application/json".toMediaType())
            ...
            logDebug("[NexoApiClient] PUT error: ${e.message}")
            Result.failure(e)
        }
    }
    ...
    suspend fun getCustomers(): Result<List<Customer>> {
        ...
        return if (bootstrap.isSuccess) {
            ...
            logDebug("[NexoApiClient] Got ${customers.size} customers from bootstrap")
    ...
    suspend fun getProducts(): Result<List<Product>> {
        ...
        return if (bootstrap.isSuccess) {
            ...
            logDebug("[NexoApiClient] Got ${mobileProducts.size} products from bootstrap")
    ...
    suspend fun getPaymentMethods(): Result<List<PaymentMethod>> {
        ...
        return if (bootstrap.isSuccess) {
            ...
            logDebug("[NexoApiClient] Got ${methods.size} payment methods from bootstrap")
    ...
    suspend fun updateOrder(orderId: Long, order: CreateOrderRequest): Result<OrderResponse> {
        val bodyAdapter = getAdapter<CreateOrderRequest>()
        val responseAdapter = getAdapter<OrderResponse>()
        return put("api/orders/$orderId", order, bodyAdapter, responseAdapter)
    }
    ...
    suspend fun getContainerTypes(): Result<ContainerTypesResponse> {
        val adapter = getAdapter<ContainerTypesResponse>()
        return get("api/mobile/containers/types", adapter)
    }
    ...
    suspend fun getCustomerContainerBalances(
        ...
        limit: Int = 100,
        offset: Int = 0
    ): Result<CustomerContainerBalancesResponse> {
        return try {
            val params = mutableListOf<String>()
            params.add("limit=$limit")
            ...
            val endpoint = if (params.isNotEmpty()) {
                "api/mobile/containers/customers/balances?${params.joinToString("&")}"
            } else {
                "api/mobile/containers/customers/balances"
            }

            val adapter = getAdapter<CustomerContainerBalancesResponse>()
            get(endpoint, adapter)
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    suspend fun receiveContainers(request: ContainerReceiveRequest): Result<ContainerReceiveResponse> {
        val bodyAdapter = getAdapter<ContainerReceiveRequest>()
        val responseAdapter = getAdapter<ContainerReceiveResponse>()
        return post("api/mobile/containers/receive", request, bodyAdapter, responseAdapter)
    }
    ...
    suspend fun deleteOrder(orderId: Long): Result<Unit> {
        return try {
            ...
            logDebug("[NexoApiClient] DELETE $url")
            ...
            response.use { resp ->
                if (resp.isSuccessful) {
                    logDebug("[NexoApiClient] Order deleted successfully")
                    Result.success(Unit)
                } else {
                    logDebug("[NexoApiClient] Delete failed: ${resp.code} ${resp.message}")
                    Result.failure(Exception("HTTP ${resp.code}: ${resp.message}"))
                }
            }
        } catch (e: Exception) {
            logDebug("[NexoApiClient] Delete error: ${e.message}")
            Result.failure(e)
        }
    }
    ...
    suspend fun getRegisters(): Result<List<Register>> {
        return try {
            logDebug("[NexoApiClient] Parsing registers manually...")
            val adapter = getAdapter<List<Map<String, Any>>>()
            val result = get("api/cash-registers", adapter)
            
            result.map { registersList ->
                logDebug("[NexoApiClient] Got ${registersList.size} registers to parse manually")
                registersList.map { registerMap ->
                    parseRegisterFromMap(registerMap)
                }
            }
        } catch (e: Exception) {
            logDebug("[NexoApiClient] Failed to parse registers: ${e.message}")
            e.printStackTrace()
            Result.failure(e)
        }
    }
    ...
    suspend fun syncRegister(registerId: Int): Result<Unit> {
        val body = emptyMap<String, Any>()
        val bodyAdapter = getAdapter<Map<String, Any>>()
        val responseAdapter = getAdapter<Map<String, Any>>()
        val result = post("api/cash-registers/sync/$registerId", body, bodyAdapter, responseAdapter)
    ...
    suspend fun getUsedRegister(): Result<Register?> {
        return try {
            ...
            logDebug("[NexoApiClient] No register in use: ${e.message}")
            Result.success(null)
        }
    }
    ...
    private fun isDebugLoggingEnabled(): Boolean {
        return System.getProperty("nexopos.debug") == "true" ||
            System.getenv("NEXOPOS_DEBUG") == "1"
    }
