...
class RegisterRepositoryImpl(
    ...
    override suspend fun getRegister(id: Int): Register {
        return withContext(Dispatchers.IO) {
            apiClient.syncRegister(id).getOrNull()
    ...
    override suspend fun getUsedRegister(): Register? {
        return withContext(Dispatchers.IO) {
            try {
                val usedRegister = apiClient.getUsedRegister().getOrNull()
                if (usedRegister != null) {
                    apiClient.syncRegister(usedRegister.id).getOrNull()
                    apiClient.getRegister(usedRegister.id).getOrNull() ?: usedRegister
                } else {
                    null
                }
            } catch (e: Exception) {
    ...
    override suspend fun getSessionHistory(registerId: Int): List<RegisterHistory> {
        return withContext(Dispatchers.IO) {
            apiClient.syncRegister(registerId).getOrNull()
